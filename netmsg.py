...
Controls:
  /send <ip> <message>   Send a message to an IP
  /history               Show message history
  /clear                 Clear the screen
  /quit                  Exit
  Or just type:  <ip> <message>
"""
...
def print_banner(local_ip: str) -> None:
    ...
    print(f"    {YELLOW}<ip> <message>{R}           — send a message")
    print(f"    {YELLOW}/send <ip> <message>{R}     — same as above")
...
def log(direction: str, peer_ip: str, peer_name: str, text: str) -> None:
    entry = {
        "time": ts(),
...
def print_incoming(peer_ip: str, peer_name: str, text: str) -> None:
    tag   = f"{MAGENTA}▶ {peer_name} ({peer_ip}){R}"
    stamp = f"{DIM}[{ts()}]{R}"
...
def print_outgoing(peer_ip: str, text: str) -> None:
    stamp = f"{DIM}[{ts()}]{R}"
    tag   = f"{GREEN}◀ you → {peer_ip}{R}"
    print(f"{stamp} {tag}: {WHITE}{text}{R}")
...
def handle_client(conn: socket.socket, addr: tuple) -> None:
    ...
    try:
        ...
        if text:
            log("in", peer_ip, peer_name, text)
            print_incoming(peer_ip, peer_name, text)
...
def send_message(target_ip: str, text: str) -> None:
    payload = json.dumps({"name": my_name, "text": text}).encode("utf-8")
    try:
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.settimeout(5)
        s.connect((target_ip, my_port))
        s.sendall(payload)
        s.close()
        log("out", target_ip, my_name, text)
        print_outgoing(target_ip, text)
    except socket.timeout:
        print_error(f"Connection to {target_ip}:{my_port} timed out.")
    except ConnectionRefusedError:
        print_error(f"{target_ip}:{my_port} refused the connection. Is netmsg running there?")
    except OSError as e:
...
def show_history() -> None:
    ...
    for e in snap:
        ...
        if e["direction"] == "in":
            who = f"{MAGENTA}{e['peer_name']} ({e['peer_ip']}){R}"
            arrow = "▶"
        else:
            who = f"{GREEN}you → {e['peer_ip']}{R}"
            arrow = "◀"
...
def repl() -> None:
    while True:
        ...
        if line.startswith("/send "):
            ...
            if len(parts) < 2 or not is_valid_ip(parts[0]):
                print_error("Usage: /send <ip> <message>")
                continue
            send_message(parts[0], parts[1])
            continue

        # shorthand: <ip> <message>
        parts = line.split(" ", 1)
        if len(parts) == 2 and is_valid_ip(parts[0]):
            send_message(parts[0], parts[1])
            continue

        print_error(f"Unknown command or bad IP. Type /quit to exit.")
...
def main() -> None:
    ...
    # Start listener
    t = threading.Thread(target=server_thread, args=(my_port,), daemon=True)
    t.start()
